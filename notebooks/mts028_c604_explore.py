...
def _(AML_DISEASE, np, pd, plt):
    ...
    def pick_quantile_example(
        ...
        row = (
            subset.iloc[(subset["auc"] - target).abs().argsort()]
            .iloc[0]
            .copy()
        )
            ...
            feature_rows.append(
                {
                    ...
                    "dynamic_range_growth": float(
                        growth[0] - np.nanmin(growth)
                    ),
    ...
    def plot_lineage_comparison(summary: pd.DataFrame):
        ...
        colors = [
            "#b2182b" if g == "AML" else "#4c78a8"
            for g in plot_df["analysis_group"]
        ]
        ax.barh(
        ...
        ax.set_ylabel("")
        ax.set_title(
            "Lineage comparison with AML separated from other Myeloid models"
        )
    ...
    def plot_aml_waterfall(aml_df: pd.DataFrame):
        plot_df = (
            aml_df.dropna(subset=["auc"])
            .sort_values("auc")
            .reset_index(drop=True)
        )
        fig, ax = plt.subplots(figsize=(11, 4.5))
        ax.bar(
            np.arange(len(plot_df)), plot_df["auc"], color="#b2182b", alpha=0.9
        )
        ax.axhline(
            plot_df["auc"].median(), color="black", linestyle="--", linewidth=1
        )
        ax.set_ylabel("Fitted AUC (lower = more sensitive)")
        ax.set_xlabel("AML models, sorted by AUC")
        ax.set_title(
            "AML waterfall shows broad heterogeneity in C-604 response"
        )
        ax.set_xticks([])
        return fig
    ...
    def plot_representative_curves(curve_df: pd.DataFrame):
        ...
        for ax, (title, labels) in zip(axes, panel_map.items(), strict=False):
            for label in labels:
                sub = curve_df.loc[
                    curve_df["example_label"].eq(label)
                ].sort_values("dose_um")
                ax.plot(
                    sub["dose_um"],
                    sub["growth_fraction"],
    ...
    def plot_curve_features(features_df: pd.DataFrame):
        ...
        palette = {"AML": "#b2182b", "Non-AML": "#4c78a8"}
        for ax, (col, title) in zip(axes.flat, feature_cols, strict=False):
            data = [
                features_df.loc[
                    features_df["aml_status"].eq("AML"), col
                ].dropna(),
                features_df.loc[
                    features_df["aml_status"].eq("Non-AML"), col
                ].dropna(),
            ]
            bp = ax.boxplot(
                data, tick_labels=["AML", "Non-AML"], patch_artist=True
            )
            for patch, label in zip(
                bp["boxes"], ["AML", "Non-AML"], strict=False
            ):
                patch.set_facecolor(palette[label])
                patch.set_alpha(0.75)
            ax.set_title(title)
        fig.suptitle(
            "First-pass curve-shape features from dose-level data", y=1.02
        )
        fig.tight_layout()
        return fig
...
def _(build_curve_features, doses):
    ...
    feature_summary = curve_features.groupby("aml_status", as_index=False).agg(
        n=("model_id", "size"),
...
def _(
    ...
    representative_curve_plot = plot_representative_curves(
